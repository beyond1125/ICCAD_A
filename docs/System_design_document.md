...
2026-07-12(P2-9)分析 cone 語意對齊(官方 Q&A A21.2):`count_fanin`/`count_fanout`/`get_fanin_cone`/`get_fanout_cone`/`count_gates_in_cone` 新增 `--stop_at_dff`(預設 1)——組合 cone 語意,邊界 DFF 計入不穿越;`0` 給舊穿越式(debug/parity 用,LLM 不暴露)。**輸出字串為破壞性變更**(本節「輸出字串即 API」條款的實例):`Fanin Gates: N` → `Fanin Gates (combinational cone, DFF Q = PI): N`(fanout 同型),`get_*_cone` 首行改為 `Combinational fanin/fanout cone of ...`,`count_gates_in_cone` JSON 多 `semantics` 欄位;唯一的下游解析者 `check_answers.py` 已同步改為容忍括號註記的 regex。transform 側 `fanin_cone_gates` 不受影響。
...
| cone 語意雙軌（分析組合 / transform 穿越） | 2026-07-12 | 官方 Q&A A21.2 裁決分析類 cone/深度僅計組合邏輯（DFF Q = PI）；但若 transform 也依此語意,暫存器輸出的扇入 cone 為空,「轉換 cone 中所有閘」等請求會無事可做（`docs/OPEN_QUESTIONS.md` D4） | **分析** action（count_fanin/count_fanout/get_fanin_cone/get_fanout_cone/count_gates_in_cone）預設停在 DFF 邊界（邊界 DFF 計入不穿越,C++ `--stop_at_dff`,P2-9）,與 oracle 對齊;**transform** cone（`Graph::fanin_cone_gates`,供 convert_cone_to_basis 等）維持穿越正反器（沿 D 輸入回走,visited-set 防護回饋環）,得到實際計算該輸出的邏輯,等價性由 cec 把關 |
