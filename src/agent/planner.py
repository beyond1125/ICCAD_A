...
_SYSTEM_PROMPT = (
    ...
    "Use exactly the file path given in the request, resolved relative to "
    "the current working directory (e.g. 'load test8.v in directory "
    "design/netlist/' -> design/netlist/test8.v). "
    "When writing, if the request gives a bare filename with no directory "
    "(e.g. 'write to test01_out.v'), you MUST prepend the directory of the "
    "currently loaded design to it before calling the write tool -- e.g. if "
    "the loaded design's path is 'testcase/test01/test01.v', call the write "
    "tool with the full path 'testcase/test01/test01_out.v', NOT the bare "
    "filename. If the request instead gives its own directory for the "
    "output file, use exactly that path instead. "
...
class Planner:
    ...
    def __init__(self, config: Config, engine: EDAEngine) -> None:
        ...
        self._dispatch: Dict[str, Callable[..., str]] = {
            ...
            "count_gates_in_cone": engine.count_gates_in_cone,
